...
void GenerateHammingSecretKey(seal::SecretKey& sk, int hwt, bool is_ntt,
                              bool is_binary, const RtPtr context) {
  ...
  for (size_t j = 0; j < coeff_modulus_size; j++) {
    ...
    std::function<uint64_t(bool)> preprocessor;
    if (is_binary) {
      preprocessor = [](bool) -> uint64_t { return 1U; };
    } else {
      preprocessor = [xor_one, neg_one](bool b) -> uint64_t {
        // b = true -> c = 0xFF -> one
    ...
    for (size_t i = 0; i < hwt; ++i) {
      dst_ptr[picked[i]] = preprocessor(rnd2[i]);
    }
...
void SKInit(seal::SecretKey& sk_ntt, seal::SecretKey& sk_non_ntt, int hwt,
  ...
  GenerateHammingSecretKey(sk_ntt, hwt, /*is_ntt*/ false, /*is_binary*/ true, rt);
...
double SymDec(const Ctx_t ct, const seal::SecretKey& sk_non_ntt,
              const RtPtr rt) {
  const auto& modulus =
      rt->last_context_data()->parms().coeff_modulus().front();
